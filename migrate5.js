...
const migrationBlock = `                this.state = JSON.parse(stored);
                ...
                if (!localStorage.getItem('allowances_extracted_v5')) {
                    ...
                    localStorage.setItem('allowances_extracted_v5', 'true');
                    this.save();
                }
`;

datajs = datajs.replace(/                this\.state = JSON\.parse\(stored\);\n\n                if \(\!localStorage\.getItem\('allowances_extracted_v4'\)\) \{[\s\S]*?this\.save\(\);\n                \}/, migrationBlock);

fs.writeFileSync('app/js/data.js', datajs);
console.log('Migrated data.js with v5');
